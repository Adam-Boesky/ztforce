"""Network regression tests: ztforce g-band lightcurves vs. PS1 DR2 photometry.
...
Bright star: RA=130.13113 Dec=+19.69525 (field near Praesepe/M44)
  PS1 DR2 reference: g=15.824 ± 0.003 (65 detections, non-variable)

Faint star: RA=130.086221 Dec=+19.735330 (same Praesepe field)
  PS1 DR2 reference: g=19.820 ± 0.011 (13 detections, non-variable)
"""
...
# Faint star (g~20): relaxed tolerances due to low SNR
_RA_FAINT = 130.086221
_DEC_FAINT = 19.735330
_PS1_G_FAINT = 19.820
_MAG_TOL_FAINT = 0.20  # larger offset allowed at low SNR
...
def test_gband_lightcurve_faint_star(tmp_path):
    """Median g-band magnitude from ztforce is within 0.20 mag of PS1 DR2 for a ~20 mag star."""
    from ztforce import build_config, run_forced_photometry
    ...
    median_g = float(np.median(detections["mag"].values))
    assert abs(median_g - _PS1_G_FAINT) < _MAG_TOL_FAINT, (
        f"Median g={median_g:.3f} deviates from PS1 g={_PS1_G_FAINT:.3f} by "
        f"{abs(median_g - _PS1_G_FAINT):.3f} mag (tolerance={_MAG_TOL_FAINT})"
    )
